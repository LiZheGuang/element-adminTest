			      ...
			      <template slot-scope="props">
			        <el-form label-position="left" inline class="demo-table-expand">
			          ...
			          <el-form-item label="商品名称">
			            <span>{{ props.row.orderTitle }}</span>
			          </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            async getOrders(){
                ...
                console.log(Orders)
                this.tableData = [];
                orderData.orderlist.forEach((item, index) => {
                    const tableData = {};
                    tableData.id = item._id;
                    tableData.total_amount = item.orderPrice / 100;
                    tableData.status = isStatus(item.status)
                    tableData.userName = item.userId.nickName;
                    tableData.amount = item.amount
                    tableData.orderTitle = item.orderTitle;
                    tableData.profile = item.profile;
                    tableData.goodsId = item.abbrId.goodsId
                    this.tableData.push(tableData);
                })
                function isStatus(keyStatus){
                    // 0未支付 1支付 2未支付取消订单 3发起退款 
                    if(keyStatus === 0 ){
                        return '未支付'
                    }else if(keyStatus === 1){
                        return '支付'
